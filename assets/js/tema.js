  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    $$(".menu-principal a").forEach((a) => {
      if (
        a.getAttribute("href") &&
        path !== "/" &&
        a.getAttribute("href").indexOf(path) !== -1
      ) {
    ...
    if (contenido && indiceLista) {
      ...
      indiceLista.addEventListener("click", (e) => {
        if (e.target.tagName === "A") {
          const dest = $(e.target.getAttribute("href"));
          if (dest) {
            e.preventDefault();
            window.scrollTo({
              top: dest.getBoundingClientRect().top + window.scrollY - 90,
              behavior: "smooth",
            });
            $$(".leccion-indice a").forEach((x) =>
    ...
    if (filtro && grid) {
      filtro.addEventListener("submit", (e) => {
        e.preventDefault();
        const fd = new FormData(filtro);
        const params = new URLSearchParams(fd).toString();
        grid.setAttribute("aria-busy", "true");
        grid.style.opacity = "0.5";
        fetch(
          window.location.origin + "/wp-json/academia-pro/v1/cursos?" + params
        )
          .then((r) => r.json())
          .then((data) => {
            grid.innerHTML = "";
            if (data.cursos && data.cursos.length) {
              data.cursos.forEach((c) => {
                ...
                card.className = "curso-tarjeta";
                card.innerHTML = `<div class="curso-tarjeta__media"><a href="${c.url}"><img src="${c.thumb || ""}" alt=""></a></div><div class="curso-tarjeta__body"><h3 class="curso-tarjeta__titulo"><a href="${c.url}">${c.titulo}</a></h3><div class="curso-tarjeta__meta"><span>${c.nivel || "General"}</span><span>${c.fecha}</span></div><div class="curso-tarjeta__progreso" data-progreso-id="${c.id}"></div></div>`;
    ...
    function cargarProgresos() {
      $$("[data-progreso-id]").forEach((holder) => {
        const id = holder.getAttribute("data-progreso-id");
        fetch(`/wp-json/academia-pro/v1/progreso/${id}`)
          .then((r) => {
          ...
          .then((d) => {
            if (!d) return;
            holder.innerHTML = `<div class="barra-progreso"><div class="barra-progreso__relleno" style="width:${d.progreso}%"></div></div>`;
          });
      });
    }
    cargarProgresos();
    ...
    ["tutor_tab_changed", "learndash_topic_loaded"].forEach((ev) =>
